...
def generate(ax_main, power_grid, voltages, frequencies):
    ...
    line1, = ax_main.plot(voltages * VOLTS_TO_MUT, d_power_d_voltage, color=deriv_color, linestyle='-',
                          label=deriv_label + '(f = $f_c$)', linewidth=2.5)
    ...
    line2, = ax4_dual.plot(voltages * VOLTS_TO_MUT, trace_power, color=FREQ_LINE_COLOR, label='$S_{21}$(f = $f_c$)',
                           linewidth=3)
